...
def _take_note(_: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    note = action_input.get("note", "")
    if not isinstance(note, str) or not note.strip():
        return ToolExecutionResult(ok=False, content={"error": "note must be a non-empty string."})
    return ToolExecutionResult(ok=True, content={"status": "noted"})
...
def create_default_tool_registry() -> ToolRegistry:
    specs = {
        ...
        "take_note": ToolSpec(
            name="take_note",
            description=(
                "Record a finding into your working memory. Use this to remember "
                "schema details, field definitions, join keys, business rules, or "
                "intermediate results. Notes persist across all subsequent steps."
            ),
            input_schema={"note": "Column 'status' uses 1=active, 0=inactive"},
        ),
    }
    handlers = {
        "answer": _answer,
