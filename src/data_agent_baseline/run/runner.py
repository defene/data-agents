...
from data_agent_baseline.agents.runtime import AgentRuntimeState, build_run_result_dict
...
def _read_progress_snapshot(snapshot_path: Path) -> dict[str, Any] | None:
...
def _run_single_task_core(
    *,
    ...
    if snapshot_path is not None:
        ...
        def progress_callback(state: AgentRuntimeState) -> None:
            _write_progress_snapshot(snapshot_path, task_id, state)
...
def _run_single_task_in_subprocess(
    ...
    try:
        ...
        logger.exception("Subprocess failed for task %s", task_id)
        queue.put(
            {
                "ok": False,
                "error": str(exc),
...
def _run_single_task_with_timeout(*, task_id: str, config: AppConfig, run_output_dir: Path) -> dict[str, Any]:
    ...
    while perf_counter() < deadline:
        ...
        poll_timeout = min(0.5, max(remaining, 0.01))
        try:
            result = queue.get(timeout=poll_timeout)
            break
        except Empty:
    ...
    if process.is_alive():
        process.kill()
        process.join()
    snapshot_payload = _read_progress_snapshot(snapshot_path)
    return _failure_run_result_payload(
        task_id,
